...
create trigger prodXtrigger
    ...
    FOR EACH ROW
    begin
        insert into inventories (exist_inv, product_id, office_id)
        value (0, new.id, 1);
        insert into inventories (exist_inv, product_id, office_id)
        value (0, new.id, 2);
        insert into inventories (exist_inv, product_id, office_id)
...
#     select product_id, inventories.id as 'inventorie id', nameProduct, exist_inv, nameOffice, nom_talla from inventories
#         join products p on inventories.product_id = p.id
#         join offices o on inventories.office_id = o.id
